...
def in_h_range_points(points, m, n, fov):
  ...
  return np.logical_and(np.arctan2(
      n, m) > (-fov[1] * np.pi / 180), np.arctan2(n, m) < (-fov[0] * np.pi / 180))
...
def velo3d_2_camera2d_points(points, v_fov, h_fov, vc_path, cc_path, mode='02'):
  ...
  """
  xy_i - 3D velodyne points corresponding to h, v FOV in the image(pixel) coordinates before
  scale adjustment
...
def print_projection_cv2(points, color, image, black_background=False):
...
def print_projection_plt(points, color, image, black_background=False):
  """ project converted velodyne points into camera image """
  if black_background:
    image = np.zeros_like(image)
  hsv_image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
