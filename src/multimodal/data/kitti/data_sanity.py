...
def _check_frame(drive_date, drive_number, frame):
  ...
  path_attack = paths.attack.processed_tensor(drive_date, drive_number, frame)

  path_arr = [path_rgb, path_attack, path_depth]
  if not (path_rgb.exists() and path_depth.exists() and path_attack.exists()):
    for f in path_arr:
      try:
        f.unlink()
